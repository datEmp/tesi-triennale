...
\begin{document}
    ...
    \begin{lstlisting}[style=ES6, title={Esempio renderizzazione componente}]
      ...
      \begin{lstlisting}[style=ES6, title={Esempio uso di useEffect()}]
        ...
        \newline
        Gli smart contracts sono stati realizzati seguendo il modello "\emph{Factory pattern}", in cui un oggetto "fabbrica" crea oggetti "prodotti". In questo caso il prodotto della fabbrica è la vendita di monete.
        Sono stati creati, perciò, due smart contracts: \verb|SaleFactory|, ovvero la fabbrica, e \verb|TokenSale|, ovvero lo schema del prodotto. Una vendita rappresenta, quindi, lo scambio di monete tra due utenti: da un lato è presente
        il venditore, ovvero colui che ha creato la vendita e offre delle monete in cambio di altre, dall'altro lato è presente il compratore, ovvero colui che accetta i termini della vendita e completa lo scambio.
        Per evitare errori ed eventuali abusi del sistema da parte di utenti, sono state poste delle restrizioni nella gestione delle vendite. In dettaglio si è impedita la creazione di vendite.
